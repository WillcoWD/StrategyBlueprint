...
async function getInitBestValues(testResults) {
  ...
  res['data']['_parseTime_'] = Math.round((new Date() - startTime) / 1000 * 10) / 10
    ...
    res['data'] = await tv.getEVData(res['data'])
  resData = res['data']
  if (res['error'] === null)
    resData = await calculateAdditionValuesToReport(resData)
...
backtest.getTestIterationResult = async (testResults, propVal, isIgnoreError = false, isIgnoreSetParam = false) => {
  try {
    ...
    if (res.error === null || isIgnoreError) {
      res['data'] = await calculateAdditionValuesToReport(res['data'])
    } else {
...
async function calculateAdditionValuesToReport(data) {
    data = await tv.getEVData(data)
    return data
}
