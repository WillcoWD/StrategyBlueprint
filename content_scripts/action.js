...
action.uploadSignals = async () => {
  await file.upload(signal.parseTSSignalsAndGetMsg, `Please check if the ticker and timeframe are set like in the downloaded data and click on the parameters of the "test" script to automatically enter new data on the chart.`, true)
}
...
action._getTestParams = async (request, strategyData, allRangeParams, paramRange, cycles, isDeepTest=false) => {
  ...
  const testMethod = options.hasOwnProperty('optMethod') && typeof (options.optMethod) === 'string' ? options.optMethod.toLowerCase() : 'annealing'
  ...
  testParams.showFullResults = options.hasOwnProperty('showFullResults') ? options.showFullResults : false
